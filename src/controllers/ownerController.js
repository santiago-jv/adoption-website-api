...
OwnerController.registerOwner = async (request, response,next) => {
    let {name, email, photo, departament, town, address, password, phoneNumber} = request.body;

    try {
        password = await hash(password,10)
        const owner = await Owner.create({name, email, photo, departament, town ,address, password, phoneNumber})
